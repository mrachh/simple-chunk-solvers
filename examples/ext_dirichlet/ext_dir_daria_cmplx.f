      ...
      complex *16, allocatable :: zsrccoefs(:,:,:)
      complex *16, allocatable :: zsrcvals(:,:,:)
      real *8 pars(1000)
      ...
      external fcurve,h2d_combc,fcurve1
...
c
      ...
      allocate(zsrcvals(8,k,maxc),zsrccoefs(6,k,maxc))
...
c
      do i=1,nch
        do j=1,k
          do ii = 1,8
             zsrcvals(ii,j,i) = srcvals(ii,j,i)
          enddo
          do ii = 1,6
             zsrccoefs(ii,j,i) = srccoefs(ii,j,i)
          enddo
        enddo
      enddo
c
...
ccc      if (2.ne.3) goto 111
      ...
      call helm2d_comb_dir_matc(k,nch,n,zsrcvals,zsrccoefs,zpars,xmat)

      call cpu_time(t2)
C$      t2 = omp_get_wtime()      
      
      call prin2('matrix generation time=*',t2-t1,1)
      call prin2('xmat is=*',xmat,20)
